...
class ConvBNRelu(nn.Module):
    def __init__(self, in_planes, out_planes, stride, kernel_size, padding, relu=True):
        ...
        self.conv = Conv2d(in_planes, out_planes, kernel_size=kernel_size, stride=stride, padding=padding, bias=False)
    ...
    def __init__(self, in_channels, mid_channels, downsample, width=1, pool_residual=False):
        ...
        if downsample:
            if pool_residual:
                ...
                conv = ConvBNRelu(in_channels, self.out_channels, stride=1, kernel_size=1,
        ...
        ## Compute FLOPs for gradmax
        # ratio = 4
        # print("mid_channels / ratio", mid_channels / ratio)
        # print("mid_channels // ratio", mid_channels // ratio)
        # self.in_conv = ConvBNRelu(
        #     in_channels, mid_channels // ratio, kernel_size=1,
        #     stride=1, padding=0)
        # self.mid_conv = ConvBNRelu(
        #     mid_channels // ratio, mid_channels // ratio, kernel_size=3,
        #     stride=(2 if downsample else 1), padding=1)
        # self.out_conv = ConvBNRelu(
        #     mid_channels // ratio, self.out_channels, kernel_size=1,
        #     stride=1, padding=0, relu=False)
        # self.out_relu = nn.ReLU(inplace=True)
...
def normal_wrn_imagenet(num_classes, args):
    ...
    width = args.wide
    mid_width = 1
    channels = (int(64/4 * width), int(128/4 * width), int(256/4 * width), int(512/4 * width))
    model = ResNet(BottleneckBlock, module_sizes=(3, 4, 6, 3), module_channels=channels,
                  num_classes=num_classes, width=mid_width,
           pool_residual=False)
...
if __name__ == '__main__':
    ...
    args = Dummy()
    args.wide = 4
    model = normal_wrn_imagenet(1000, args)
    analyze_model(model)

    inputs = torch.randn(1, 3, 224, 224)
    flops = compute_flops(model, inputs)
    # print(f"flops: {round(flops/1e9,3)} GLOPs")
    print(f"flops: {round(flops):,} GLOPs")
    # 2640453376; 2644545280